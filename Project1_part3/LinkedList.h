...
LinkedList<T>::~LinkedList(){
    ...
    while(head != nullptr){   //while current node (head) is not null (iterate through the list)
    ...
    head = nullptr; //assign head as null
}
...
LinkedList<T>::LinkedList(const LinkedList<T>& list){
    ...
    Node* currNode = list.head;   //assign current node to parameter list's head
    this->tail = list.tail;    //assign current tail to parameter list's tail
...
//Reference for writing this loop: TA Alexis Doughtery
    while(currNode != nullptr){     //while current node is not null (iterate through the list)
        ...
        AddTail(currData);     //add to head of list the current data
        currNode = currNode->next;   //assign current node to current node's next node
...
LinkedList<T> &LinkedList<T>::operator=(const LinkedList<T>& rhs) {
    ...
    if(this != &rhs) {  //1.don't self assign
        ...
        Node* currNode = rhs.head;   //assign current node to parameter list's head
        this->tail = rhs.tail;    //assign current tail to parameter list's tail
        this->nodeCounter = 0;    //assign current number of nodes to 0
        T currData;   //declare a variable for current node's data
...
void LinkedList<T>::InsertAfter(LinkedList::Node* node, const T& data) {
    ...
    while(nextNode != node){  //while current node's next node is not the node given in parameter
